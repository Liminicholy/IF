...
class InstFifoIO extends Bundle {
  ...
  // 读取端口
  val read_en1      = Input(Bool()) //id
  val read_en2      = Input(Bool()) //id 只需要一个push
